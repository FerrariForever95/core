...
class Logger:
    ...
    def log(self, level, message, source="GENERAL"):
        self._write(f"[{self._time_str()}] [SRC:{source}] [{self.LEVELS.get(level, 'UNKNOWN')}] {message}")
...
# ---------------- PID types -----------------------
# PIDs are 4 digits: TYPE (1 digit, thousands place) + 3 random digits.
# The type digit is never 0, so every pid is genuinely 4 digits
# (1000-9999), and just by looking at a pid you know what kind of
# process it is -- similar in spirit to how Linux reserves low PIDs
# for kernel threads, just made explicit instead of incidental.
#
#   1xxx  KERNEL   -- boot/UI/core system processes, owner=root
#   2xxx  USER     -- ordinary cooperative apps (loop/periodic/once)
#   3xxx  THREAD   -- real _thread-backed concurrent tasks
