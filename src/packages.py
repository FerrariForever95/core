...
from filesystem import FileManager, NotADirectoryError
from security import usermanager
from system import Disk

# Extensions PackageManager knows how to *import as a live module* after
# download. .bin (and anything else not listed here) is treated as an
# opaque asset: it gets downloaded, verified, and placed on disk under
# its own real extension, but is never passed to __import__/install(pm).
_IMPORTABLE_EXT = (".py", ".mpy")
...
class PackageManager:
    ...
    def _module_name_for(self, record):
        """
        Derive an importable module name for a package record, or None
        if this package isn't an importable Python module at all.

        Packages are dropped under install_path as <name><ext>. .py and
        .mpy are both real importable module types on MicroPython (a
        precompiled .mpy imports exactly like a .py of the same name --
        MicroPython's importer picks whichever is present). Anything
        else (.bin, or no extension) is an opaque asset -- e.g. a
        firmware/data blob -- and has no module name to import.
        """
        filename = record.get("filename", "")
        for ext in _IMPORTABLE_EXT:
    ...
    def _ensure_on_path(self, install_path):
        ...
        Delegates to core.SysPathManager (the single shared
    ...
    def _load_and_install_module(self, name, record):
        ...
        if module_name is None:
            filename = record.get("filename", "")
            if "." in filename:
                # Known asset type (e.g. .bin) with no module to import --
                # it's already on disk at install_path, which is all an
                # asset package needs. Nothing to register as a command.
                self.logger.debug(
                    "'{}' is a non-importable asset ({}); nothing to load.".format(name, filename),
                    source=self.source,
                )
                return True
    ...
    def _make_pkglist_entry(self, entry):
        name = entry.get("name")
        ext = self._ext_of(entry.get("file") or "") or ".py"
        return {
            ...
            "install_path": self._bin_dir_for(name), "filename": "{}{}".format(name, ext),
    ...
    def _download_package(self, entry):
        ...
        # <name><ext> -- where <ext> is whatever extension the source
        # file in the repo actually has (.py, .mpy, .bin, ...) -- so
        # /bin/<name>/<name><ext> is guaranteed either way. Forcing
        # everything to ".py" here used to silently corrupt .mpy/.bin
        # packages: a precompiled .mpy or a binary blob would get
        # written out with a .py extension and then get fed to
        # __import__ as if it were Python source.
        name = entry.get("name")
        target_dir = self._bin_dir_for(name)
        source_file = entry.get("file") or ""
        ext = self._ext_of(source_file) or ".py"

        ok = self.git.download(entry.get("author"), entry.get("repository"), source_file,
                                branch=entry.get("branch"), save_dir=target_dir)
        if not ok:
            return False

        source_basename = source_file.split("/")[-1]
        desired_path = "{}/{}{}".format(target_dir, name, ext)
        actual_path = "{}/{}".format(target_dir, source_basename) if source_basename else desired_path
        ...
        if not self._verify_download(desired_path, entry):
            try:
    ...
    def _ext_of(self, path):
        """Return the real extension (with leading '.') of a repo file
        path, e.g. 'bin/foo/foo.mpy' -> '.mpy'. Empty string if none."""
        base = (path or "").split("/")[-1]
        if "." not in base:
            return ""
        return "." + base.split(".")[-1]

    def _verify_download(self, path, entry):
        """Verify a freshly-downloaded file actually landed correctly,
        by byte size and (when the catalog provides one) checksum --
        not by assuming success, and not by any text/char-count logic,
        which silently breaks for binary .mpy/.bin content.

        - size: if the catalog entry specifies one (in bytes), the
          file on disk must match exactly.
        - sha256: if the catalog entry specifies one (hex string), it's
          recomputed from the file on disk in fixed-size chunks (safe
          for large .bin/.mpy files on a low-memory device) and must
          match exactly.
        - If neither is provided, we fall back to just confirming the
          file exists and is non-empty -- still real verification of
          the actual bytes on disk, just without a known-good target
          to compare against.
        """
        try:
            actual_size = os.stat(path)[6]
        except OSError as e:
            self._error("Verification failed: could not stat '{}': {}".format(path, e))
            return False

        expected_size = entry.get("size")
        if expected_size is not None and actual_size != expected_size:
            self._error(
                "Verification failed for '{}': expected {} bytes, got {}".format(path, expected_size, actual_size)
            )
            return False
                ...
                with open(path, "rb") as f:
                    while True:
                        ...
                        if not chunk:
                            break
                        h.update(chunk)
                actual_hex = "".join("{:02x}".format(b) for b in h.digest())
            except Exception as e:
                self._error("Verification failed: could not hash '{}': {}".format(path, e))
                return False
            if actual_hex.lower() != expected_sha256.lower():
                self._error(
                    "Verification failed for '{}': checksum mismatch (expected {}, got {})".format(
                        path, expected_sha256, actual_hex
                    )
                )
                return False

        if actual_size == 0:
            self._error("Verification failed for '{}': file is empty".format(path))
            return False
